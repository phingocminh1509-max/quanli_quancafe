...
typedef struct { 
    ...
    char maPhong[10]; char loaiPhong[20]; 

    float giaTien; int trangThai; char maKH[10]; 

    time_t checkIn; time_t checkOut; 
...
typedef struct { 
    ...
    char maKH[10]; char tenKH[50]; char sdt[15]; 
...
Phong* taoPhong() { 
    ...
    printf("Loai phong: "); scanf(" %[^\n]", p->loaiPhong); 

    printf("Gia tien: "); scanf("%f", &p->giaTien); 

    getchar(); p->trangThai = 0; strcpy(p->maKH, ""); 
...
int tonTaiPhong(Phong* head, char ma[]) { 
    ...
    while (head != NULL) { 
        ...
        if (strcmp(head->maPhong, ma) == 0) return 1; 
...
int tonTaiKH(KhachHang* head, char ma[]) { 
    ...
    while (head != NULL) { 
        ...
        if (strcmp(head->maKH, ma) == 0) return 1; 
...
void hienThiPhong(Phong* head) { 
    ...
    printf("\n%-10s %-15s %-10s %-10s %-10s", "MaPhong","Loai","Gia","TrangThai","MaKH"); 

    while (head != NULL) { 

        printf("\n%-10s %-15s %-10.2f %-10s %-10s", 

            head->maPhong, head->loaiPhong, head->giaTien, 
...
void hienThiKH(KhachHang* head) { 
    ...
    printf("\n%-10s %-25s %-15s", "MaKH","TenKH","SDT"); 

    printf("\n-----------------------------------------------------"); 

    while (head != NULL) { 

        printf("\n%-10s %-25s %-15s", head->maKH, head->tenKH, head->sdt); 
...
// thuePhong - đã bổ sung đầy đủ logic bị thiếu 
...
void thuePhong(Phong* dsPhong, KhachHang* dsKH) { 
    ...
    printf("Nhap ma KH: "); scanf("%9s", maKH); 
...
void traPhong(Phong* head) { 
    ...
    if (p == NULL) { printf("Khong tim thay phong!\n"); return; } 

    if (p->trangThai == 0) { printf("Phong dang trong!\n"); return; } 

    p->checkOut = time(NULL); 

    double soGiay = difftime(p->checkOut, p->checkIn); 

    // Tính số ngày: dùng ceil để làm tròn lên, tối thiểu 1 ngày 

    int soNgay = (soGiay <= 0) ? 1 : (int)ceil(soGiay / 86400.0); 
    ...
    p->trangThai = 0; p->maKH[0] = '\0'; 

    p->checkIn = 0; p->checkOut = 0; 

    printf("Da tra phong thanh cong!\n"); 
...
void suaPhong(Phong* head) { 
    ...
    printf("Gia moi: "); scanf("%f", &p->giaTien); getchar(); 
...
void xoaPhong(Phong** head) { 
    ...
    printf("Nhap ma phong can xoa: "); scanf("%9s", ma); getchar(); 
    ...
    if (curr == NULL) { printf("Khong tim thay!\n"); return; } 

    if (curr->trangThai == 1) { printf("Phong dang duoc thue, khong the xoa!\n"); return; } 
...
void timKiemPhong(Phong* head) { 
    ...
    printf("Nhap ma phong: "); scanf("%9s", ma); getchar(); 
...
void hoanDoi(Phong* a, Phong* b) { 
    ...
    float giaTien; int trangThai; time_t checkIn, checkOut; 

    strcpy(maPhong, a->maPhong); strcpy(loaiPhong, a->loaiPhong); 

    strcpy(maKH, a->maKH); giaTien = a->giaTien; 

    trangThai = a->trangThai; checkIn = a->checkIn; checkOut = a->checkOut; 

    strcpy(a->maPhong, b->maPhong); strcpy(a->loaiPhong, b->loaiPhong); 

    strcpy(a->maKH, b->maKH); a->giaTien = b->giaTien; 

    a->trangThai = b->trangThai; a->checkIn = b->checkIn; a->checkOut = b->checkOut; 

    strcpy(b->maPhong, maPhong); strcpy(b->loaiPhong, loaiPhong); 

    strcpy(b->maKH, maKH); b->giaTien = giaTien; 

    b->trangThai = trangThai; b->checkIn = checkIn; b->checkOut = checkOut; 
...
void sapXepPhong(Phong* head) { 
    ...
    for (Phong* i = head; i != NULL; i = i->next) 
        ...
        for (Phong* j = i->next; j != NULL; j = j->next) 
            ...
            if (i->giaTien > j->giaTien) hoanDoi(i, j); 
...
// luuPhong - đã thêm kiểm tra fopen 

void luuPhong(Phong* head) { 

    FILE* f = fopen("phong.dat", "wb"); 

    if (f == NULL) { printf("Loi: Khong the mo file!\n"); return; } 
    ...
    while (head != NULL) { 
        ...
        temp.giaTien = head->giaTien; temp.trangThai = head->trangThai; 

        strcpy(temp.maKH, head->maKH); 

        temp.checkIn = head->checkIn; temp.checkOut = head->checkOut; 
...
void docPhong(Phong** head) { 
    ...
    while (fread(&temp, sizeof(PhongFile), 1, f)) { 
        ...
        strcpy(p->maPhong, temp.maPhong); strcpy(p->loaiPhong, temp.loaiPhong); 

        p->giaTien = temp.giaTien; p->trangThai = temp.trangThai; 

        strcpy(p->maKH, temp.maKH); 

        p->checkIn = temp.checkIn; p->checkOut = temp.checkOut; 
...
void luuKH(KhachHang* head) { 
    ...
    while (head != NULL) { 
        ...
        strcpy(temp.maKH, head->maKH); strcpy(temp.tenKH, head->tenKH); 
...
void docKH(KhachHang** head) { 
    ...
    while (fread(&temp, sizeof(KhachHangFile), 1, f)) { 
        ...
        strcpy(k->maKH, temp.maKH); strcpy(k->tenKH, temp.tenKH); 
...
void menu() { 
    ...
    Phong* dsPhong = NULL; KhachHang* dsKH = NULL; 

    docPhong(&dsPhong); docKH(&dsKH); 
    ...
    do { 
        ...
        switch (chon) { 
            ...
            case 0: 
                ...
                luuPhong(dsPhong); luuKH(dsKH); 

                giaiPhongDSPhong(dsPhong); giaiPhongDSKH(dsKH); 
